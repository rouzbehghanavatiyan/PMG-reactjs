    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="relative w-full md:w-64">
          <Search
            size={18}
            className={`absolute top-1/2 transform -translate-y-1/2 text-gray-500 ${dir === "rtl" ? "right-3" : "left-3"}`}
          />
          <input
            type="text"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredNews.map((news) => (
          <div
            key={news.id}
            className="bg-bmw-surface border border-bmw-border rounded-xl overflow-hidden flex flex-col hover:shadow-xl hover:shadow-black/10 transition-all duration-300 group"
          >
            ...
            <div className="p-5 flex flex-col flex-1">
              <div className="flex-1">
                <h3 className="text-lg font-bold text-bmw-text mb-2 group-hover:text-bmw-blue transition-colors">
                  {news.title}
                </h3>
                <p className="text-bmw-textSec text-sm line-clamp-3">
                  {news.summary}
                </p>
              </div>
