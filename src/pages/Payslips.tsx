...
const Payslips: React.FC = () => {
  ...
  const paidOnText = isFa ? `پرداخت شده در ${paymentDate}` : `Paid on ${paymentDate}`;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
         <div>
          <h1 className="text-2xl font-bold text-bmw-text">{t('salary_title')}</h1>
        ...
        <button className="flex items-center gap-2 bg-bmw-surface border border-bmw-border px-4 py-2 rounded hover:border-bmw-blue transition-colors">
           ...
           <span className="text-sm font-medium text-bmw-text">{t('password_protected')}</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-bmw-surface border border-bmw-border rounded-lg p-6 shadow-sm">
          <h3 className="text-lg font-bold text-bmw-text mb-6">{t('income_overview')}</h3>
            ...
            <ResponsiveContainer width="100%" height="100%" minWidth={0}>
              <BarChart data={data}>
                ...
                <XAxis 
                  ...
                  tick={{fill: theme === 'dark' ? '#999' : '#6B7280', fontSize: 12, fontFamily: isFa ? 'Vazirmatn' : 'Inter'}} 
                  axisLine={false} 
                  tickLine={false} 
                />
                <YAxis 
                ...
                <Tooltip 
                  ...
                  formatter={(value: number) => [formatCurrency(value), t('table.size')?.replace('Size', 'Amount') || 'Amount']}
                  labelStyle={{ textAlign: isFa ? 'right' : 'left' }}
                />
                <Bar dataKey="total" radius={[4, 4, 0, 0]} barSize={40}>
                  {data.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={index === data.length - 1 ? '#0066B1' : (theme === 'dark' ? '#4A4A4A' : '#9CA3AF')} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="bg-bmw-blue rounded-lg p-6 text-white flex flex-col justify-between shadow-xl shadow-blue-900/20 relative overflow-hidden">
          ...
          <div>
            <p className="text-blue-100 font-medium mb-1">{t('last_net_salary')}</p>
          ...
          <div className="space-y-3 mt-8">
            <div className="flex justify-between text-sm border-b border-white/20 pb-2">
              ...
              <span className="font-semibold" dir="ltr">{formatCurrency(latest.base)}</span>
            </div>
            <div className="flex justify-between text-sm border-b border-white/20 pb-2">
              <span className="text-blue-100">{t('overtime_bonus')}</span>
              <span className="font-semibold text-green-300" dir="ltr">{formatCurrency(latest.bonus, true)}</span>
            </div>
            <div className="flex justify-between text-sm pb-2">
              <span className="text-blue-100">{t('deductions')}</span>
              <span className="font-semibold text-red-300" dir="ltr">{formatCurrency(-latest.deductions)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-bmw-surface border border-bmw-border rounded-lg overflow-hidden shadow-sm">
        <div className="p-4 border-b border-bmw-border flex justify-between items-center">
          <h3 className="text-lg font-bold text-bmw-text">{t('history_title')}</h3>
          <button className="text-sm text-bmw-blue hover:text-blue-400 transition-colors">{t('view_history')}</button>
        </div>
        <div className="divide-y divide-bmw-border">
          {data.slice().reverse().map((item, idx) => (
            <div key={idx} className="p-4 flex items-center justify-between hover:bg-bmw-hover transition-colors group">
              <div className="flex items-center gap-4">
                <div className="p-2 bg-bmw-base rounded text-bmw-textSec group-hover:text-bmw-blue transition-colors">
                  <FileText size={20} />
                </div>
                <div>
                  <p className="text-bmw-text font-medium">Salary Slip - {item.month} 2023</p>
                  <p className="text-xs text-bmw-textSec">{t('processed_via')}</p>
                </div>
              </div>
              <div className="flex items-center gap-6">
                 <span className="text-bmw-text font-mono hidden md:block" dir="ltr">
