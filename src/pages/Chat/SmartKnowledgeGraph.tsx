...
export const SmartKnowledgeGraph: React.FC = () => {
  ...
  const handleStartEditDoc = async (doc: GraphDoc) => {
    ...
    try {
      const res = await fetch(`/api/graph/documents/${doc.id}/details`);
      if (!res.ok)
        throw new Error(
          isRtl
      ...
      const commRes = await fetch(`/api/graph/documents/${doc.id}/communities`);
      if (commRes.ok) {
  ...
  const handleTriggerCommunityDetection = async () => {
    ...
    try {
      const res = await fetch(
        `/api/graph/documents/${editingDoc.id}/communities`,
        {
          method: "POST",
        },
      );
  ...
  const handleTriggerEntityResolution = async () => {
    ...
    try {
      const res = await fetch(
        `/api/graph/documents/${editingDoc.id}/entity-resolution`,
        {
          method: "POST",
        },
      );
      ...
      if (data.success) {
        ...
        const detailRes = await fetch(
          `/api/graph/documents/${editingDoc.id}/details`,
        );
        if (detailRes.ok) {
  ...
  const handleSaveDocDetails = async () => {
    ...
    try {
      const res = await fetch(
        `/api/graph/documents/${editingDoc.id}/update-graph`,
        {
          method: "POST",
  ...
  const handleReaction = async (
    ...
    try {
      await fetch("/api/graph/chat/message/reaction", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const fetchSessions = async () => {
    try {
      const res = await fetch(
        `/api/graph/chat/sessions?userId=${activeUserId}`,
      );
      const data = await res.json();
  ...
  const handleViewMessageGraph = async (msg: Message, index: number) => {
    ...
    try {
      const res = await fetch(`/api/graph/chat/message/${msg.id}/subgraph`);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchChatHistory = async (sessionId: string) => {
    try {
      const res = await fetch("/api/graph/chat/history", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleToggleDoc = async (docId: number, isEnabled: boolean) => {
    try {
      const res = await fetch(`/api/graph/documents/${docId}/toggle`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleToggleStrict = async (docId: number, strictMode: boolean) => {
    try {
      const res = await fetch(`/api/graph/documents/${docId}/strict-mode`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const confirmDeleteDoc = async () => {
    ...
    try {
      const res = await fetch(`/api/graph/documents/${docId}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.success) {
  ...
  const confirmDeleteSession = async () => {
    ...
    try {
      const res = await fetch(`/api/graph/chat/sessions/${sessId}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.success) {
