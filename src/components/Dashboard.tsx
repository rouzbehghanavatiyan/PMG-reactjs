...
import { Bell, ArrowUpRight, Search, Printer, Calendar, MessageSquare, ExternalLink } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  // Adaptive Notifications Data
  const notificationsData = {
    en: [
      { text: 'Your leave request for Nov 12 has been approved.', time: '2 hours ago' },
      { text: 'New IT security policy update available.', time: '5 hours ago' },
      { text: 'Meeting reminder: Sales Team Weekly at 2 PM.', time: '1 day ago' }
    ],
    fa: [
      { text: 'درخواست مرخصی شما برای ۲۱ آبان تایید شد.', time: '۲ ساعت پیش' },
      { text: 'به‌روزرسانی جدید خط‌مشی امنیت IT در دسترس است.', time: '۵ ساعت پیش' },
      { text: 'یادآوری جلسه: جلسه هفتگی تیم فروش ساعت ۱۴:۰۰.', time: '۱ روز پیش' }
    ]
  };
  ...
  const currentNotifications = notificationsData[language === 'fa' ? 'fa' : 'en'];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-bmw-text tracking-tight">{t('welcome')}</h1>
        ...
        <div className="flex gap-3">
          ...
          >
             <img src="/assets/1002.jpg" alt="User" className="w-full h-full object-cover" />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { labelKey: 'surveys', icon: '📋', descKey: 'surveys_desc', link: '/surveys' },
        ].map((action, idx) => (
          <div 
            key={idx} 
          ...
          >
            <div className="flex justify-between items-start mb-3">
              ...
              <ArrowUpRight size={16} className="text-bmw-textSec group-hover:text-bmw-blue transition-colors rtl:rotate-180" />
            </div>
            <h3 className="text-bmw-text font-semibold">{t(`quick_actions.${action.labelKey}`)}</h3>
            <p className="text-xs text-bmw-textSec mt-1">{t(`quick_actions.${action.descKey}`)}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-bmw-text">{t('latest_news')}</h2>
            <button 
          ...
          <div className="grid gap-6">
            {currentNews.map((news) => (
              <div key={news.id} className="bg-bmw-surface border border-bmw-border rounded-lg overflow-hidden flex flex-col md:flex-row hover:shadow-xl hover:shadow-black/10 transition-shadow">
                ...
                <div className="p-5 flex flex-col justify-between flex-1">
                  <div>
                    <h3 className="text-lg font-bold text-bmw-text mb-2">{news.title}</h3>
                    <p className="text-bmw-textSec text-sm line-clamp-2">{news.summary}</p>
                  </div>
                  <div className="flex items-center justify-between mt-4">
                    <span className="text-xs text-bmw-textSec">{news.date}</span>
                    <button 
                      onClick={() => navigate(`/news/${news.id}`)}
                      className="text-sm text-bmw-text flex items-center gap-1 hover:text-bmw-blue transition-colors"
                    >
                      {t('read_more')} <ArrowUpRight size={14} className="rtl:rotate-180" />
                    </button>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-bmw-surface border border-bmw-border rounded-lg p-5 shadow-sm">
            ...
            <div className="space-y-4">
              {currentNotifications.map((note, i) => (
                <div key={i} className="flex gap-3 items-start pb-3 border-b border-bmw-border last:border-0 last:pb-0">
                  <div className="w-2 h-2 rounded-full bg-bmw-blue mt-2 shrink-0" />
                  <div>
                    <p className="text-sm text-bmw-textSec">{note.text}</p>
                    <span className="text-xs text-bmw-textSec opacity-70 mt-1 block">{note.time}</span>
                  </div>
                </div>
              ))}
            </div>
